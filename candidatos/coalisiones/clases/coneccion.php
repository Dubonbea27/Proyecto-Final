...
$dsn = 'mysql:dbname=voto;127.0.0.1';
$usuario = 'root';
$clave = '';
...
function consultaA($coneccion, $sql)
{
	...
	try{
		...
		switch ($condicion)
		{
			...
			case "UPDATE":
			$msgerror = "No se ha podido Actualizar";
			$msgok = "datos Actualizados";
			break;
			default:
			$msgerror = "es posiblenque no se use un estandar de consulta sql";
			break;
	...
	if ($objeto != NULL) {
		if (is_array($objeto)){
			...
			foreach ($objeto as $datos){
				...
				if ($editar !="0"){
					$tabla .= "<td><a href=". $form. "Buscar.php?id_coalisiones=".$datos['id_coalisiones'].">Modificar</a></td>";
